...
set_loading(false);
set_error(getLabel('errorApply', 'Failed to apply action code. Please try again.'));
...
set_loading(false);
set_error(getLabel('errorRemove', 'Failed to remove action code. Please try again.'));
...
return (
  ...
  <div className="w-full bg-white p-6 rounded-lg shadow space-y-3"><h2 className="text-lg font-bold">{title()}</h2>{_isMounted ? (
  <>{hasAppliedCode() ? (
  ...
  <button  type="button" className="text-violet-600 hover:text-violet-800 text-sm font-medium transition-colors disabled:opacity-50"  onClick={(event) => handleRemove() }  disabled={_loading}>{getLabel('remove', 'Remove')}</button>
) : null}</div>
) : null}
{!hasAppliedCode() ? (
  <div className="flex gap-2"><input  type="text" className="flex-1 text-sm border border-gray-300 rounded-md px-3 py-2 focus:ring-2 focus:ring-violet-500 focus:border-transparent disabled:opacity-50"  value={_code}  onChange={(e) => {
set_code(e.target.value);
} }  onKeyDown={(e) => handleKeyDown(e) }  placeholder={getLabel('placeholder', 'Enter action code')}  disabled={_loading}  /><button  type="button" className="bg-violet-600 text-white text-sm font-medium px-4 py-2 rounded-md hover:bg-violet-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed whitespace-nowrap"  onClick={(event) => handleApply() }  disabled={_loading || !_code.trim()}>{_loading ? (
