...
import type { CartStartVariables } from "propeller-sdk-v2/dist/service/CartService";

function AddToCart(props: AddToCartProps) {
  const [quantity, setQuantity] = useState<AddToCartState["quantity"]>(() => 1);

  const [loading, setLoading] = useState<AddToCartState["loading"]>(
    () => false
  );

  const [success, setSuccess] = useState<AddToCartState["success"]>(
    () => false
  );

  const [modalVisible, setModalVisible] = useState<
    AddToCartState["modalVisible"]
  >(() => false);

  const [activeCartId, setActiveCartId] = useState<
    AddToCartState["activeCartId"]
  >(() => "");

  const [toastMessage, setToastMessage] = useState<
    AddToCartState["toastMessage"]
  >(() => "");
  ...
  > {
    return (
      (props.product as any)?.media?.images?.items?.[0]?.imageVariants?.[0]
        ?.url || ""
    );
  }
  ...
  function getProductPrice(): ReturnType<AddToCartState["getProductPrice"]> {
    const price =
      props.price !== undefined
        ? props.price
        : (props.product as any)?.price?.gross;
