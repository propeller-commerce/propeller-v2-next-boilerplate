...
function getCategoryUrl(cat: Category, index: number): ReturnType<BreadcrumbsState["getCategoryUrl"]>{
if (props.getUrl) {
...
return (
  ...
  <nav  aria-label="Breadcrumb"  className={`breadcrumbs ${props.className as string || ''}`}><ol className="flex flex-wrap items-center text-sm text-muted-foreground">{props.showHome !== false ? (
  <li className="flex items-center"><a className="hover:text-foreground transition-colors"  href={props.homeUrl as string || '/'}>{getLabel('home', 'Home')}</a></li>
) : null}{getDisplayItems()?.map((cat, index) => (
  <li className="flex items-center"  key={cat.categoryId || index}>{showSeparatorBefore(index) ? (
  <span  aria-hidden="true" className="mx-2 select-none text-muted-foreground/40">{getLabel('separator', '/')}</span>
) : null}{isCurrentItem(index) ? (
  <a className="hover:text-foreground transition-colors"  href={getCategoryUrl(cat, index)}>{getCategoryName(cat)}</a>
) : null}{!isCurrentItem(index) ? (
  <a className="hover:text-foreground transition-colors"  href={getCategoryUrl(cat, index)}>{getCategoryName(cat)}</a>
) : null}</li>
))}</ol></nav>
