...
return (
  ...
  <div className="relative"><div className="relative"  onMouseEnter={(event) => {
setIsHovered(true);
} }  onMouseLeave={(event) => {
setIsHovered(false);
...
<div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">{getItems().length === 0 ? (
  ...
  <>{getItems()?.map((item) => (
  <div className="flex gap-3"  key={item.itemId}><div className="w-20 h-20 flex-shrink-0 bg-gray-50 rounded-md overflow-hidden border border-gray-100 flex items-center justify-center">{getItemImageUrl(item) ? (<img className="w-full h-full object-contain p-2"  src={getItemImageUrl(item)}  alt={getItemName(item)}  onError={(e) => { const img = e.target as HTMLImageElement; img.style.display = 'none'; const parent = img.parentElement; if (parent && !parent.querySelector('svg')) { parent.innerHTML = '<svg class="w-8 h-8 text-gray-300" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M2.25 15.75l5.159-5.159a2.25 2.25 0 013.182 0l5.159 5.159m-1.5-1.5l1.409-1.409a2.25 2.25 0 013.182 0l2.909 2.909M3.75 21h16.5A2.25 2.25 0 0022.5 18.75V5.25A2.25 2.25 0 0020.25 3H3.75A2.25 2.25 0 001.5 5.25v13.5A2.25 2.25 0 003.75 21z" /></svg>'; } }}  />) : (<svg className="w-8 h-8 text-gray-300" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M2.25 15.75l5.159-5.159a2.25 2.25 0 013.182 0l5.159 5.159m-1.5-1.5l1.409-1.409a2.25 2.25 0 013.182 0l2.909 2.909M3.75 21h16.5A2.25 2.25 0 0022.5 18.75V5.25A2.25 2.25 0 0020.25 3H3.75A2.25 2.25 0 001.5 5.25v13.5A2.25 2.25 0 003.75 21z" /></svg>)}</div><div className="flex-1 min-w-0 flex flex-col justify-between py-1"><div><div className="flex justify-between items-start gap-2"><a className="text-sm font-medium leading-tight text-gray-900 hover:text-violet-600 transition-colors line-clamp-2"  href={getItemProductUrl(item)}  onClick={(event) => closeSidebar() }>{getItemName(item)}</a><span className="font-semibold text-sm text-gray-900 whitespace-nowrap">
