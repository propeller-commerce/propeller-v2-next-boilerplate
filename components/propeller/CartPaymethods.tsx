...
function showLogo(): ReturnType<CartPaymethodsState["showLogo"]>{
...
function isOnAccountMethod(method: CartPaymethod): ReturnType<CartPaymethodsState["isOnAccountMethod"]>{
...
function getLogoUrl(method: CartPaymethod): ReturnType<CartPaymethodsState["getLogoUrl"]>{
const code = (method.code || '').toLowerCase();
...
return (
  ...
  <div  key={method.code}  onClick={(event) => handleSelect(method) }  className={`cursor-pointer border border-gray-200 rounded-lg p-4 flex flex-col gap-2 transition-all ${selectedCode === method.code ? 'border-violet-600 bg-violet-50 shadow-sm' : 'hover:border-violet-300'}`}><div className="flex justify-between items-center"><div className="flex items-center gap-2">{showLogo() && getLogoUrl(method) ? (
