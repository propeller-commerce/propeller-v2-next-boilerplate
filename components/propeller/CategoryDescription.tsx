...
const match = props.category.description.find((d: LocalizedString) => d.language === props.language);
return match?.value || '';
}
...
return (
  ...
  <button className="mt-2 text-sm font-medium text-primary hover:underline"  onClick={(event) => toggle() }>{expanded ? (
  <>Read less</>
) : null}{!expanded ? (
  <>Read more</>
) : null}</button>
