...
return (
  ...
  <div className="relative aspect-square overflow-hidden bg-gray-50 p-4"><a className="block h-full w-full"  href={getClusterUrl()}  onClick={(e) => handleClusterClick(e) }>{!!getClusterImageUrl() ? (
...
) : null}<div className="flex flex-1 flex-col gap-2 p-4">{props.showSku !== false && !!getClusterSku() ? (
  <div className="font-mono text-xs text-gray-400">{getClusterSku()}</div>
) : null}{props.showName !== false ? (
  <a className="line-clamp-2 text-sm font-medium leading-tight text-gray-900 transition-colors hover:text-violet-600"  href={getClusterUrl()}  onClick={(e) => handleClusterClick(e) }>{getClusterName()}</a>
  ...
  <div className="mt-auto pt-2"><span className="text-lg font-bold text-gray-900">{getClusterPrice()}</span></div>
...
) : null}</div><div className="px-4 pb-4"><a className="flex w-full items-center justify-center rounded-md bg-violet-600 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-violet-700 focus:outline-none focus:ring-2 focus:ring-violet-500 focus:ring-offset-2"  href={getClusterUrl()}  onClick={(e) => handleClusterClick(e) }>{getLabel('viewCluster', 'View cluster')}</a></div></div>
