...
function ClusterCard(props: ClusterCardProps) {
  const [isFavorite, setIsFavorite] = useState<ClusterCardState["isFavorite"]>(
    () => false
  );
  ...
  > {
    return (
      ...
      (props.cluster as Cluster)?.defaultProduct?.shortDescriptions?.[0]
        ?.value ||
      ""
    );
  }
  ...
  function getStockQuantity(): ReturnType<
    ClusterCardState["getStockQuantity"]
  > {
  ...
  return (
    ...
    >
        ...
        <div className="relative aspect-square overflow-hidden bg-gray-50 p-4">
          ...
          computedImageLabels().length > 0 ? (
            <div className="pointer-events-none absolute left-2 top-2 flex flex-col gap-1">
              {computedImageLabels()?.map((label, i) => (
                <span key={i} className="inline-block rounded bg-violet-600 px-2 py-0.5 text-xs font-medium text-white shadow-sm">
                  {label}
                </span>
              ))}
            </div>
      ...
      <div className="flex flex-1 flex-col gap-2 p-4">
        ...
        computedTextLabels().length > 0 ? (
          <div className="flex flex-col gap-0.5">
            {computedTextLabels()?.map((item) => (
              <div key={item.name} className="text-xs text-gray-500">{item.value}</div>
            ))}
          </div>
        ) : null}
        ...
        {props.showStock !== false && getStockQuantity() >= 0 ? (
          <div className="flex items-center gap-1.5">
            <span
              className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${getStockStatusClass()}`}
            >
              {getStockStatusLabel()}
            </span>
            {getStockQuantity() > 0 ? (
