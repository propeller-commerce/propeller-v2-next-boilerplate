...
interface ClusterConfiguratorState {
 ...
 handleAttributeSelect: (settingName: string, value: string) => void;
   ...
   const [selectedAttributes, setSelectedAttributes] = useState<ClusterConfiguratorState["selectedAttributes"]>(() => ({}))
...
return settings.slice().sort((a: ClusterConfigSetting, b: ClusterConfigSetting) => parseInt(a.priority) - parseInt(b.priority));
}


function attributeNameMatches(attr: AttributeResult, targetName: string): ReturnType<ClusterConfiguratorState["attributeNameMatches"]>{
const attrName = attr.attributeDescription?.descriptions?.[0]?.value || attr.attributeDescription?.name;
return attrName === targetName || attr.attributeDescription?.name === targetName || (attr.attributeDescription?.descriptions?.some((desc: any) => desc.value === targetName) ?? false);
}


function extractAttributeValues(attr: AttributeResult): ReturnType<ClusterConfiguratorState["extractAttributeValues"]>{
...
function getAttributeDisplayName(attributeName: string): ReturnType<ClusterConfiguratorState["getAttributeDisplayName"]>{
...
if (Array.isArray(attributeItems)) {
 const matchingAttr = (attributeItems as AttributeResult[]).find((attr: AttributeResult) => attributeNameMatches(attr, attributeName));
...
function getAttributeValues(attributeName: string): ReturnType<ClusterConfiguratorState["getAttributeValues"]>{
...
function getAvailableValuesForIndex(attributeName: string, settingIndex: number): ReturnType<ClusterConfiguratorState["getAvailableValuesForIndex"]>{
return getAvailableValuesForIndexWithSelections(attributeName, settingIndex, selectedAttributes as Record<string, string>);
}


function getAvailableValuesForIndexWithSelections(attributeName: string, settingIndex: number, selections: Record<string, string>): ReturnType<ClusterConfiguratorState["getAvailableValuesForIndexWithSelections"]>{
if (settingIndex === 0) {
...
return sortedSettings.map((setting: ClusterConfigSetting, index: number) => {
 ...
 const isPreviousSelectionMissing = index > 0 && sortedSettings.slice(0, index).some((prev: ClusterConfigSetting) => !sel[prev.name]);
...
function findMatchingProduct(selections: Record<string, string>): ReturnType<ClusterConfiguratorState["findMatchingProduct"]>{
const products = props.products as Product[] || [];
...
function handleAttributeSelect(settingName: string, value: string): ReturnType<ClusterConfiguratorState["handleAttributeSelect"]>{
const sortedSettings = getSortedSettings();
const changedIndex = sortedSettings.findIndex((s: ClusterConfigSetting) => s.name === settingName);
...
const matchingAttr = (attrItems as AttributeResult[]).find((attr: AttributeResult) => attributeNameMatches(attr, setting.name));
if (matchingAttr) {
...
if (matchingProduct) {
 ...
 return (
   ...
   <div  className={`cluster-configurator ${props.className || ''}`}>{!!(props.config as ClusterConfig)?.settings?.length ? (
   <div className="configurator-content flex flex-col gap-6">{getSettingsWithValues()?.map((setting) => (
   <div className="attribute-group"  key={setting.id}><h4 className="font-semibold text-sm text-gray-700 mb-3">{setting.displayName || setting.name}</h4>{setting.displayType === 'DROPDOWN' ? (
   <select className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500 disabled:bg-gray-50 disabled:text-gray-400 cursor-pointer"  value={setting.selectedValue}  disabled={setting.disabled}  onChange={(e) => handleAttributeSelect(setting.name, e.target.value) }><option  value="">{getLabel('selectOption', '— Select —')}</option>{setting.availableValues?.map((val) => (
   <option  key={val}  value={val}>{val}</option>
 ))}</select>
 ) : null}{setting.displayType === 'RADIO' ? (
   <div className="flex flex-wrap gap-2">{setting.availableValues?.map((val) => (
   <label  key={val}  className={`flex items-center gap-2 px-3 py-1.5 rounded-lg border text-sm font-medium transition-colors select-none ${setting.disabled ? 'opacity-50 cursor-not-allowed border-gray-200 text-gray-400' : setting.selectedValue === val ? 'border-violet-500 bg-violet-50 text-violet-700 cursor-pointer' : 'border-gray-200 text-gray-700 hover:border-violet-300 cursor-pointer'}`}><input  type="radio" className="sr-only"  name={`cluster-${props.clusterId}-${setting.name}`}  value={val}  checked={setting.selectedValue === val}  disabled={setting.disabled}  onChange={(event) => handleAttributeSelect(setting.name, val) }  />{val}</label>
 ))}</div>
 ) : null}{setting.displayType === 'COLOR' ? (
   <div className="flex flex-wrap gap-2">{setting.availableValues?.map((val) => (
   <button  type="button"  key={val}  title={val}  disabled={setting.disabled}  onClick={(event) => handleAttributeSelect(setting.name, val) }  style={{
backgroundColor: val
 ...
 ) : null}{setting.displayType === 'IMAGE' ? (
   <div className="flex flex-wrap gap-3">{setting.availableValues?.map((val) => (
   <button  type="button"  key={val}  disabled={setting.disabled}  onClick={(event) => handleAttributeSelect(setting.name, val) }  className={`relative w-16 h-16 rounded-lg border-2 overflow-hidden transition-all ${setting.disabled ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'} ${setting.selectedValue === val ? 'border-violet-600 ring-2 ring-violet-300 ring-offset-1' : 'border-gray-200 hover:border-violet-300'}`}><img className="w-full h-full object-cover"  src={val}  alt={val}  />{setting.selectedValue === val ? (
   <div className="absolute inset-0 bg-violet-600 bg-opacity-20 flex items-center justify-center"><svg  fill="currentColor"  viewBox="0 0 20 20" className="w-5 h-5 text-violet-700"><path  fillRule="evenodd"  d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z"  clipRule="evenodd"  /></svg></div>
 ) : null}</button>
 ))}</div>
 ) : null}{setting.displayType !== 'DROPDOWN' && setting.displayType !== 'RADIO' && setting.displayType !== 'COLOR' && setting.displayType !== 'IMAGE' ? (
   <div className="flex flex-wrap gap-2">{setting.availableValues?.map((val) => (
   <button  type="button"  key={val}  disabled={setting.disabled}  onClick={(event) => handleAttributeSelect(setting.name, val) }  className={`px-3 py-1.5 rounded-lg border text-sm font-medium transition-colors ${setting.disabled ? 'opacity-50 cursor-not-allowed border-gray-200 text-gray-400' : setting.selectedValue === val ? 'border-violet-500 bg-violet-50 text-violet-700 cursor-pointer' : 'border-gray-200 text-gray-700 hover:border-violet-300 cursor-pointer'}`}>{val}</button>
 ))}</div>
 ) : null}</div>
 ))}</div>
 ) : null}</div>
