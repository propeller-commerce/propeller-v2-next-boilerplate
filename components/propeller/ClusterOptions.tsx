...
interface ClusterOptionsState {
 ...
 handleOptionChange: (optionIdStr: string, productIdStr: string) => void;
   ...
   const [selectedProductIds, setSelectedProductIds] = useState<ClusterOptionsState["selectedProductIds"]>(() => ({}))
...
function getProductImageUrl(product: Product): ReturnType<ClusterOptionsState["getProductImageUrl"]>{
...
return options.filter((option: ClusterOption) => option.hidden !== Enums.YesNo.Y).map((option: ClusterOption) => {
 ...
 if (selectedProductId) {
   const selectedProduct = (option.products || []).find((p: Product) => p.productId.toString() === selectedProductId);
   if (selectedProduct) {
...
function handleOptionChange(optionIdStr: string, productIdStr: string): ReturnType<ClusterOptionsState["handleOptionChange"]>{
...
if (productIdStr && props.onOptionSelect) {
 ...
 const product = (option?.products || []).find((p: Product) => p.productId.toString() === productIdStr);
 if (product) {
 ...
 return (
   ...
   <div  className={`cluster-options ${props.className || ''}`}>{getOptionsForRender().length > 0 ? (
   <div className="cluster-options-content flex flex-col gap-6">{getOptionsForRender()?.map((option) => (
   <div className="option-group"  key={option.id}><div className="flex items-center gap-2 mb-2"><h4 className="font-semibold text-sm text-gray-700">{option.name}</h4>{option.isRequired ? (
   <span className="inline-flex items-center rounded-full bg-red-50 px-2 py-0.5 text-xs font-medium text-red-600 ring-1 ring-inset ring-red-500/10">{getLabel('required', 'Required')}</span>
 ) : null}</div><select  value={option.selectedProductId}  onChange={(e) => handleOptionChange(option.idStr, e.target.value) }  className={`w-full rounded-lg border px-3 py-2 text-sm focus:outline-none focus:ring-2 cursor-pointer ${option.hasError ? 'border-red-400 focus:ring-red-500' : option.isRequired ? 'border-gray-300 focus:ring-violet-500' : 'border-gray-200 focus:ring-violet-500'}`}><option  value="">{option.isRequired ? (
   <>{getLabel('selectRequired', '— Select an option —')}</>
 ) : <>{getLabel('selectOptional', '— None (Optional) —')}</>}</option>{option.products?.map((product) => (
   <option  key={product.productId}  value={product.productIdStr}>{product.label}</option>
 ))}</select>{option.hasError ? (
   <p className="mt-1 text-xs text-red-500">{getLabel('requiredError', 'This option is required')}</p>
 ) : null}{option.hasSelection ? (
   <div className="mt-3 flex items-center gap-3 rounded-lg border border-gray-100 bg-gray-50 p-3">{!!option.previewImageUrl ? (
   <img className="h-12 w-12 flex-shrink-0 rounded border border-gray-100 bg-white object-contain"  src={option.previewImageUrl}  alt={option.previewName}  />
 ) : null}{!option.previewImageUrl ? (
   <div className="flex h-12 w-12 flex-shrink-0 items-center justify-center rounded border border-gray-200 bg-gray-100"><svg  fill="none"  stroke="currentColor"  viewBox="0 0 24 24" className="h-5 w-5 text-gray-300"><path  strokeLinecap="round"  strokeLinejoin="round"  d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"  strokeWidth={1.5}  /></svg></div>
 ) : null}<div className="min-w-0 flex-1"><p className="truncate text-sm font-medium text-gray-900">{option.previewName}</p><p className="text-sm font-semibold text-violet-600">{option.previewPrice}</p></div></div>
 ) : null}</div>
 ))}</div>
 ) : null}</div>
