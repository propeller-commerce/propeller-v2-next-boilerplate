...
import { useState, useEffect } from 'react'
  import  { FavoriteListService, FavoriteList, GraphQLClient, Contact, Customer, FavoriteListsSearchInput } from 'propeller-sdk-v2';
...
const [editingListId, setEditingListId] = useState<FavoriteListsState["editingListId"]>(() => (null))


const [editListName, setEditListName] = useState<FavoriteListsState["editListName"]>(() => (''))


const [editSetAsDefault, setEditSetAsDefault] = useState<FavoriteListsState["editSetAsDefault"]>(() => (false))


const [showDeleteModal, setShowDeleteModal] = useState<FavoriteListsState["showDeleteModal"]>(() => (false))


const [listToDelete, setListToDelete] = useState<FavoriteListsState["listToDelete"]>(() => (null))


const [showCreateModal, setShowCreateModal] = useState<FavoriteListsState["showCreateModal"]>(() => (false))
...
async function handleUpdateList(listId: string): ReturnType<FavoriteListsState["handleUpdateList"]>{
...
setLists(lists.map((l: FavoriteList) => {
...
setShowCreateModal(true);
  ...
  <div className="space-y-4"><div className="space-y-2"><input  type="text"  placeholder="Enter list name" className="max-w-md block w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:ring-blue-500"  value={editListName}  onChange={(e) => {
setEditListName(e.target.value);
} }  /></div><div className="flex items-center space-x-2"><input  type="checkbox" className="rounded border-gray-300"  id={`default-edit-${list.id}`}  checked={editSetAsDefault}  onChange={(e) => {
setEditSetAsDefault(e.target.checked);
} }  /><label className="text-sm text-gray-500"  htmlFor={`default-edit-${list.id}`}>{getLabel('makeDefault', 'Make default')}</label></div><div className="flex gap-2"><button className="inline-flex items-center px-3 py-1.5 text-sm font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700 disabled:opacity-50"  onClick={(event) => handleUpdateList(String(list.id)) }  disabled={!editListName.trim()}>{getLabel('editSave', 'Save')}</button><button className="inline-flex items-center px-3 py-1.5 text-sm font-medium rounded-md border border-gray-300 text-gray-700 bg-white hover:bg-gray-50"  onClick={(event) => handleCancelEdit() }>{getLabel('editCancel', 'Cancel')}</button></div></div>
) : null}{editingListId !== String(list.id) ? (
  <div className="space-y-2"><div className="flex items-center gap-2"><button className="text-xl font-semibold hover:text-blue-600 transition-colors text-left"  onClick={(event) => props.onListClick && props.onListClick(list.id) }>{list.name}</button>{props.showDefaultIndicator !== false && list.isDefault ? (
  <span className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800">{getLabel('defaultBadge', 'Default')}</span>
) : null}</div><div className="flex items-center gap-4 text-sm text-gray-500">{props.showLastModified !== false ? (
  <div className="flex items-center gap-1"><svg  xmlns="http://www.w3.org/2000/svg"  width="16"  height="16"  viewBox="0 0 24 24"  fill="none"  stroke="currentColor"  strokeWidth="2"  strokeLinecap="round"  strokeLinejoin="round"><rect  width="18"  height="18"  x="3"  y="4"  rx="2"  ry="2"  /><line  x1="16"  x2="16"  y1="2"  y2="6"  /><line  x1="8"  x2="8"  y1="2"  y2="6"  /><line  x1="3"  x2="21"  y1="10"  y2="10"  /></svg>{getLabel('lastModified', 'Last modified')}: {formatDate(list.updatedAt)}</div>
) : null}{props.showItemsCount !== false ? (
  <div className="flex items-center gap-1"><svg  xmlns="http://www.w3.org/2000/svg"  width="16"  height="16"  viewBox="0 0 24 24"  fill="none"  stroke="currentColor"  strokeWidth="2"  strokeLinecap="round"  strokeLinejoin="round"><path  d="M16.5 9.4 7.55 4.24"  /><path  d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"  /><polyline  points="3.29 7 12 12 20.71 7"  /><line  x1="12"  x2="12"  y1="22"  y2="12"  /></svg>{getTotalCount(list)}{getLabel('items', 'items')} ({getProductCount(list)}{getLabel('products', 'products')}, {getClusterCount(list)}{getLabel('clusters', 'clusters')})
                                                        </div>
) : null}</div></div>
) : null}</div>{props.showActions !== false && editingListId !== String(list.id) ? (
  <div className="flex gap-2"><button  title="Edit" className="h-8 w-8 p-0 inline-flex items-center justify-center rounded-md text-gray-500 hover:text-gray-700 hover:bg-gray-100"  onClick={(event) => handleEditList(list) }><svg  xmlns="http://www.w3.org/2000/svg"  width="16"  height="16"  viewBox="0 0 24 24"  fill="none"  stroke="currentColor"  strokeWidth="2"  strokeLinecap="round"  strokeLinejoin="round"><path  d="M17 3a2.85 2.83 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5Z"  /><path  d="m15 5 4 4"  /></svg></button><button  title="Delete" className="h-8 w-8 p-0 inline-flex items-center justify-center rounded-md text-red-500 hover:text-red-700 hover:bg-red-50"  onClick={(event) => handleDeleteList(list) }><svg  xmlns="http://www.w3.org/2000/svg"  width="16"  height="16"  viewBox="0 0 24 24"  fill="none"  stroke="currentColor"  strokeWidth="2"  strokeLinecap="round"  strokeLinejoin="round"><path  d="M3 6h18"  /><path  d="M19 6v14c0 1-1 2-2 2H7c-1 0-2-1-2-2V6"  /><path  d="M8 6V4c0-1 1-2 2-2h4c1 0 2 1 2 2v2"  /></svg></button></div>
) : null}</div></div>
))}</div>
) : null}
{displayedLists().length === 0 ? (
  <div className="border border-gray-200 rounded-lg p-12 text-center space-y-4"><div className="bg-gray-100 p-4 rounded-full w-16 h-16 flex items-center justify-center mx-auto"><svg  xmlns="http://www.w3.org/2000/svg"  width="32"  height="32"  viewBox="0 0 24 24"  fill="none"  stroke="currentColor"  strokeWidth="2"  strokeLinecap="round"  strokeLinejoin="round" className="text-gray-400"><path  d="M19 14c1.49-1.46 3-3.21 3-5.5A5.5 5.5 0 0 0 16.5 3c-1.76 0-3.332.67-4.5 2.17C10.832 3.67 9.26 3 7.5 3A5.5 5.5 0 0 0 2 8.5c0 2.3 1.5 4.05 3 5.5l7 7Z"  /></svg></div><div><p className="text-lg font-medium">{getLabel('noLists', 'No favorite lists')}</p><p className="text-gray-500">{getLabel('noListsDescription', 'Start by creating a new list to save your items.')}</p></div>{props.allowFavoriteListCreate !== false ? (
  <button className="inline-flex items-center px-4 py-2 text-sm font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700"  onClick={(event) => {
setShowCreateModal(true);
} }>{getLabel('createFirstList', 'Create your first list')}</button>
) : null}</div>
) : null}</>
) : null}{showCreateModal ? (
  <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4"><div className="bg-white p-6 rounded-lg max-w-md w-full shadow-lg border"><div className="flex justify-between items-center mb-4"><h3 className="text-xl font-bold">{getLabel('createTitle', 'Create New List')}</h3><button className="h-8 w-8 p-0 inline-flex items-center justify-center rounded-md text-gray-500 hover:text-gray-700 hover:bg-gray-100"  onClick={(event) => {
closeCreateModal();
} }>
                            ×
                        </button></div><div className="space-y-4"><div className="space-y-2"><label className="text-sm font-medium">Name</label><input  type="text" className="block w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:ring-blue-500"  value={newListName}  onChange={(e) => {
setNewListName(e.target.value);
} }  placeholder={getLabel('createPlaceholder', 'Enter list name')}  /></div><div className="flex items-center space-x-2"><input  type="checkbox"  id="create-set-default" className="rounded border-gray-300"  checked={newSetAsDefault}  onChange={(e) => {
setNewSetAsDefault(e.target.checked);
} }  /><label  htmlFor="create-set-default" className="text-sm text-gray-500">{getLabel('setAsDefault', 'Set as default favorite list')}</label></div><div className="flex justify-end gap-3 pt-2"><button className="inline-flex items-center px-4 py-2 text-sm font-medium rounded-md border border-gray-300 text-gray-700 bg-white hover:bg-gray-50"  onClick={(event) => {
closeCreateModal();
} }>{getLabel('cancelButton', 'Cancel')}</button><button className="inline-flex items-center px-4 py-2 text-sm font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700 disabled:opacity-50"  onClick={(event) => handleCreateList() }  disabled={!newListName.trim()}>{getLabel('saveButton', 'Save')}</button></div></div></div></div>
) : null}{showDeleteModal && listToDelete ? (
  <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4"><div className="bg-white p-6 rounded-lg max-w-md w-full shadow-lg border"><div className="flex justify-between items-center mb-4"><h3 className="text-xl font-bold">{getLabel('deleteTitle', 'Delete Favorite List')}</h3><button className="h-8 w-8 p-0 inline-flex items-center justify-center rounded-md text-gray-500 hover:text-gray-700 hover:bg-gray-100"  onClick={(event) => handleCancelDelete() }>
                            ×
                        </button></div><div className="space-y-4"><p>{getLabel('deleteConfirm', 'Are you sure you want to delete')}<strong>&quot;{listToDelete?.name}&quot;</strong>?</p><p className="text-sm text-red-600">{getLabel('deleteWarning', 'This action cannot be undone.')}</p></div><div className="flex justify-end gap-3 pt-6"><button className="inline-flex items-center px-4 py-2 text-sm font-medium rounded-md border border-gray-300 text-gray-700 bg-white hover:bg-gray-50"  onClick={(event) => handleCancelDelete() }>{getLabel('cancelButton', 'Cancel')}</button><button className="inline-flex items-center px-4 py-2 text-sm font-medium rounded-md text-white bg-red-600 hover:bg-red-700"  onClick={(event) => handleConfirmDelete() }>{getLabel('deleteButton', 'Delete')}</button></div></div></div>
) : null}</div>
