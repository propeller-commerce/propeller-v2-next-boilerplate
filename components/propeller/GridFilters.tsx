...
function GridFilters(props: GridFiltersProps) {
  const [selectedFilters, setSelectedFilters] = useState<
    GridFiltersState["selectedFilters"]
  >(() => ({}));

  const [currentMin, setCurrentMin] = useState<GridFiltersState["currentMin"]>(
    () => 0
  );

  const [currentMax, setCurrentMax] = useState<GridFiltersState["currentMax"]>(
    () => 9999
  );

  const [expandedFilters, setExpandedFilters] = useState<
    GridFiltersState["expandedFilters"]
  >(() => ({}));
  ...
  function getSelectedCount(): ReturnType<
    GridFiltersState["getSelectedCount"]
  > {
    let n = 0;
  ...
  function hasActiveFilters(): ReturnType<
    GridFiltersState["hasActiveFilters"]
  > {
    const sel = selectedFilters as Record<string, string[]>;
  ...
  function isSelected(
    ...
    return (
      (selectedFilters as Record<string, string[]>)[filterName] || []
    ).includes(value);
  }
  ...
  return (
    <div
      className={`space-y-4 ${
        (props.isMobile as boolean) ? "pb-8" : "sticky top-24"
    ...
    >
      ...
      {showPriceFilter() &&
      (props.priceMin !== undefined || props.priceMax !== undefined) ? (
        <>
            ...
            <div className="flex items-center gap-2">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  max={getMaxBound()}
                  onChange={(e) =>
                    handleMinChange(parseFloat(e.target.value) || 0)
                  }
              ...
              <div className="relative flex-1">
                ...
                <input
                  ...
                  max={getMaxBound()}
                  onChange={(e) =>
                    handleMaxChange(parseFloat(e.target.value) || 0)
                  }
                  onBlur={(event) => applyPrice()}
                />
              </div>
      ...
      {getFilteredFilters()?.map((filter) => (
        <div key={getFilterName(filter)} className="border-b border-gray-100 pb-3 last:border-b-0">
          <button
            type="button"
