...
for (let i = 1; i <= total; i++) items.push({
  type: 'page',
  value: i
});
return items;
...
return (
  ...
  <div  className={`${props.className as string || ''}`}>{showPagination() ? (
  <>{(props.variant as string || 'compact') === 'compact' ? (
  <div className="flex justify-center items-center gap-2"><button  type="button" className="inline-flex items-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 shadow-sm hover:bg-gray-50 disabled:opacity-40 disabled:cursor-not-allowed"  disabled={getCurrentPage() === 1}  onClick={(event) => handlePageChange(getCurrentPage() - 1) }>{getLabel('previous')}</button><span className="px-2 text-sm font-medium text-gray-700">{getLabel('page')}&nbsp;{getCurrentPage()}&nbsp;{getLabel('of')}&nbsp;{getTotalPages()}</span><button  type="button" className="inline-flex items-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 shadow-sm hover:bg-gray-50 disabled:opacity-40 disabled:cursor-not-allowed"  disabled={getCurrentPage() === getTotalPages()}  onClick={(event) => handlePageChange(getCurrentPage() + 1) }>{getLabel('next')}</button></div>
) : null}
{(props.variant as string || 'compact') === 'full' ? (
  <div className="flex justify-center items-center gap-1 flex-wrap"><button  type="button" className="inline-flex items-center rounded-md border border-gray-300 bg-white px-3 py-2 text-sm font-medium text-gray-700 shadow-sm hover:bg-gray-50 disabled:opacity-40 disabled:cursor-not-allowed"  disabled={getCurrentPage() === 1}  onClick={(event) => handlePageChange(getCurrentPage() - 1) }>{getLabel('previous')}</button>{getFullPages()?.map((item) => (
  <div className="inline-flex">{item.type === 'dots' ? (
  <span className="inline-flex items-center justify-center min-w-[2rem] px-1 py-2 text-sm text-gray-500 select-none">
                                        ...
                                    </span>
) : null}{item.type === 'page' ? (
  <button  type="button"  onClick={(event) => handlePageChange(item.value) }  className={item.value === getCurrentPage() ? 'inline-flex items-center justify-center min-w-[2.25rem] rounded-md border border-blue-600 bg-blue-600 px-3 py-2 text-sm font-semibold text-white shadow-sm' : 'inline-flex items-center justify-center min-w-[2.25rem] rounded-md border border-gray-300 bg-white px-3 py-2 text-sm font-medium text-gray-700 shadow-sm hover:bg-gray-50'}>{item.value}</button>
) : null}</div>
))}<button  type="button" className="inline-flex items-center rounded-md border border-gray-300 bg-white px-3 py-2 text-sm font-medium text-gray-700 shadow-sm hover:bg-gray-50 disabled:opacity-40 disabled:cursor-not-allowed"  disabled={getCurrentPage() === getTotalPages()}  onClick={(event) => handlePageChange(getCurrentPage() + 1) }>{getLabel('next')}</button></div>
) : null}</>
) : null}</div>
