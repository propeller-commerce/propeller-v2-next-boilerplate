...
return isAvailable() ? getLabel('available', 'Available') : getLabel('notAvailable', 'Not available');
}


function getAvailabilityClass(): string {
return isAvailable() ? 'text-green-600 bg-green-50 border-green-100' : 'text-red-600 bg-red-50 border-red-100';
}
...
return (
  ...
  {hasInventory() ? (
  <><div  className={`flex flex-wrap items-center gap-2 ${props.className as string || ''}`}>{props.showAvailability !== false ? (
  <span  className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-0.5 text-xs font-medium ${getAvailabilityClass()}`}><span  className={`h-1.5 w-1.5 rounded-full ${getAvailabilityDotClass()}`}  />{getAvailabilityLabel()}</span>
) : null}{props.showStock !== false && !!getStockStatusLabel() ? (
  <span  className={`inline-flex items-center gap-1 rounded-full border px-2.5 py-0.5 text-xs font-medium ${getStockStatusClass()}`}>{getStockStatusLabel()}{getTotalQuantity() > 0 ? (
