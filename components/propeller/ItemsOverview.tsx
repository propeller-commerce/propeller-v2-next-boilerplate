          ...
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              {showSku() && getItemSku(item) ? (
                <p className="text-xs text-gray-500">{getItemSku(item)}</p>
              ) : null}
              {showQuantity() ? (
              ...
              {showAvailability() && getItemAvailability(item) ? (
                <p className={`text-xs ${isInStock(item) ? 'text-green-600' : 'text-red-500'}`}>
                  {getItemAvailability(item)}
                </p>
              ) : null}
            </div>
            {showPrice() ? (
              <div className="text-sm font-medium">{formatItemPrice(getItemTotalPrice(item))}</div>
            ) : null}
          </div>
        ))}
      </div>
