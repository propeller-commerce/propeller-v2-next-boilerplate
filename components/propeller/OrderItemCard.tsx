  ...
  return (
    <tbody>
      <tr className={isChildItem() ? 'border-0' : 'hover:bg-gray-50 transition'}>
        ...
        {showQuantity() ? (
          <td className={isChildItem() ? 'px-6 py-2 text-center text-sm text-gray-600' : 'px-6 py-4 text-center'}>
            {quantity()}
          </td>
        ) : null}
        {showDiscount() ? (
          <td className={isChildItem() ? 'px-6 py-2 text-right text-sm text-gray-600' : 'px-6 py-4 text-right whitespace-nowrap text-orange-600'}>
            {discount() > 0 ? formatDiscountDisplay() : null}
          </td>
        ) : null}
        {showPrice() ? (
          <td className={isChildItem() ? 'px-6 py-2 text-right whitespace-nowrap text-sm text-gray-600' : 'px-6 py-4 text-right whitespace-nowrap'}>
      ...
      {hasChildren() ? (
        <>
          {(props.childItems || []).map((child: any) => (
