...
return (
  ...
  <tbody><tr  className={isChildItem() ? 'border-0' : 'hover:bg-gray-50 transition'}><td  className={isChildItem() ? 'px-6 py-2 pl-28' : 'px-6 py-4'}><div className="flex items-center gap-4">{showImage() ? (
  <>{productImage() ? (
  <div className="relative w-16 h-16 flex-shrink-0 rounded overflow-hidden"><img className="object-cover w-full h-full"  src={productImage()}  alt={productName()}  /></div>
) : null}
{!productImage() ? (
...
) : null}<div>{titleLinkable() && productUrl() && !isChildItem() ? (
  <a className="font-medium text-blue-600 hover:underline"  href={productUrl()}>{productName()}</a>
) : null}{!titleLinkable() || !productUrl() || isChildItem() ? (
  <span  className={isChildItem() ? 'text-sm text-gray-700' : 'font-medium'}>{productName()}</span>
) : null}{showSku() && productSku() ? (
  <p className="text-sm text-gray-500 mt-1">SKU: {productSku()}</p>
) : null}{showItemNotes() && notes() ? (
  <p className="text-sm text-gray-400 mt-1 italic">{notes()}</p>
) : null}{showStockComponent() ? (
  <p className="text-xs text-gray-400 mt-1">Stock info</p>
) : null}</div></div></td>{showQuantity() ? (
  <td  className={isChildItem() ? 'px-6 py-2 text-center text-sm text-gray-600' : 'px-6 py-4 text-center'}>{quantity()}</td>
) : null}{showDiscount() ? (
  <td  className={isChildItem() ? 'px-6 py-2 text-right text-sm text-gray-600' : 'px-6 py-4 text-right whitespace-nowrap text-orange-600'}>{discount() > 0 ? (
