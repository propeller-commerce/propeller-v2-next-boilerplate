...
return (
  ...
  <div className="w-full md:w-80 bg-white p-6 rounded-lg shadow space-y-3">{showSubtotal() ? (
  ...
  <><div className="flex justify-between text-violet-600"><span>{getLabel('discount', 'Discount:')}</span><span>{discountDisplay()}</span></div>
<div className="flex justify-between text-gray-600 border-t pt-2 border-dashed"><span>{getLabel('subtotalWithDiscount', 'Subtotal with discount:')}</span><span>{formatItemPrice(subtotalWithDiscount())}</span></div></>
) : null}{hasTransactionCosts() ? (
  <div className="flex justify-between text-gray-600"><span>{getLabel('transactionCosts', 'Transaction costs:')}</span><span>{formatItemPrice(transactionCosts())}</span></div>
...
) : null}{showVATs() && taxPercentages().length > 0 ? (
  <>{taxPercentages()?.map((tax, index) => (
  <div className="flex justify-between text-gray-600 text-sm"  key={index}><span>{tax.percentage}% {getLabel('vat', 'VAT')}:</span><span>{formatItemPrice(Number(tax.total))}</span></div>
))}</>
) : null}{showTotalVat() ? (
...
) : null}<div className="flex justify-between text-xl font-bold pt-4 border-t text-gray-900 mt-2"><span>{getLabel('total', 'Total:')}</span><span>{formatItemPrice(totalInclVat())}</span></div></div>
