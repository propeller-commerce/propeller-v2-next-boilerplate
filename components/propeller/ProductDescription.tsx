...
return (
  ...
  {!!html ? (
  <><div  className={`product-description ${props.className as string || ''}`}>{!shouldTruncate() || expanded ? (
...
__html: html
  ...
  <button  type="button" className="mt-2 text-sm font-medium text-primary hover:underline"  onClick={(event) => toggle() }>{expanded ? (
  <>Read less</>
) : null}{!expanded ? (
  <>Read more</>
) : null}</button>
