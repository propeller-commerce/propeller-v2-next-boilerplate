...
return (
  ...
  <div  className={`product-tabs ${props.className as string || ''}`}><div className="flex border-b border-border">{isTabVisible('description') ? (
  <button  type="button"  onClick={(event) => selectTab('description') }  className={`px-5 py-3 text-sm font-medium transition-colors border-b-2 -mb-px ${isActive('description') ? 'border-primary text-primary' : 'border-transparent text-muted-foreground hover:text-foreground hover:border-border'}`}>{getLabel('description', 'Description')}</button>
) : null}{isTabVisible('specifications') ? (
  <button  type="button"  onClick={(event) => selectTab('specifications') }  className={`px-5 py-3 text-sm font-medium transition-colors border-b-2 -mb-px ${isActive('specifications') ? 'border-primary text-primary' : 'border-transparent text-muted-foreground hover:text-foreground hover:border-border'}`}>{getLabel('specifications', 'Specifications')}</button>
) : null}{isTabVisible('downloads') ? (
  <button  type="button"  onClick={(event) => selectTab('downloads') }  className={`px-5 py-3 text-sm font-medium transition-colors border-b-2 -mb-px ${isActive('downloads') ? 'border-primary text-primary' : 'border-transparent text-muted-foreground hover:text-foreground hover:border-border'}`}>{getLabel('downloads', 'Downloads')}</button>
) : null}{isTabVisible('videos') ? (
  ...
  <ProductVideos  videos={(props.product as Product).media?.videos as PaginatedMediaVideoResponse}  language={props.language as string || 'NL'}  labels={props.videosLabels}  />
) : null}</div></div>
