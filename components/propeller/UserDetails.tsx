...
{shouldShowCompanyInfo() && getActiveCompany() ? (
  ...
  <div className="grid grid-cols-1 gap-1"><label className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Tax Number</label><div className="font-medium">{getActiveCompany()?.taxNumber}</div></div>
) : null}{getActiveCompany()?.cocNumber ? (
  <div className="grid grid-cols-1 gap-1"><label className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">CoC Number</label><div className="font-medium">{getActiveCompany()?.cocNumber}</div></div>
) : null}</div></div>
) : null}
{shouldListCompanies() && getCompanies().length > 0 ? (
  <div className="user-details__companies rounded-lg bg-card text-card-foreground shadow-sm"><div className="p-6 pb-2"><h3 className="text-lg font-semibold">Companies</h3></div><div className="p-6 pt-2"><ul className="space-y-2">{getCompanies()?.map((company) => (
  <li  key={String(company.companyId)}  className={`flex items-center gap-2 py-2 px-3 rounded-md ${getActiveCompany()?.companyId === company.companyId ? 'bg-primary/10 font-semibold text-primary' : 'text-foreground'}`}><span className="truncate">{company.name}</span>{getActiveCompany()?.companyId === company.companyId ? (
...
{shouldShowInvoiceAddress() || shouldShowDeliveryAddress() ? (
  <div className="user-details__addresses rounded-lg bg-card text-card-foreground shadow-sm"><div className="p-6 pb-2"><h3 className="text-lg font-semibold">Default Addresses</h3></div><div className="p-6 pt-2"><div className="grid grid-cols-1 md:grid-cols-2 gap-6">{shouldShowInvoiceAddress() ? (
  <div className="space-y-3"><h4 className="text-base font-bold">Invoice Address</h4>{getDefaultInvoiceAddress() ? (
  <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200">{(getDefaultInvoiceAddress() as any)?.company ? (
  <div className="font-bold text-lg mb-1">{(getDefaultInvoiceAddress() as any)?.company}</div>
) : null}{getAddressName(getDefaultInvoiceAddress()!) ? (
  <div className="font-medium mb-1">{getAddressName(getDefaultInvoiceAddress()!)}</div>
) : null}<div className="text-gray-600">{getAddressLine1(getDefaultInvoiceAddress()!)}</div><div className="text-gray-600">{getAddressLine2(getDefaultInvoiceAddress()!)}</div>{getDefaultInvoiceAddress()?.country ? (
  <div className="text-gray-600">{getDefaultInvoiceAddress()?.country}</div>
) : null}</div>
) : null}{!getDefaultInvoiceAddress() ? (
  <p className="text-gray-500 italic">No invoice address found</p>
) : null}</div>
) : null}{shouldShowDeliveryAddress() ? (
  <div className="space-y-3"><h4 className="text-base font-bold">Delivery Address</h4>{getDefaultDeliveryAddress() ? (
  <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200">{(getDefaultDeliveryAddress() as any)?.company ? (
  <div className="font-bold text-lg mb-1">{(getDefaultDeliveryAddress() as any)?.company}</div>
) : null}{getAddressName(getDefaultDeliveryAddress()!) ? (
  <div className="font-medium mb-1">{getAddressName(getDefaultDeliveryAddress()!)}</div>
) : null}<div className="text-gray-600">{getAddressLine1(getDefaultDeliveryAddress()!)}</div><div className="text-gray-600">{getAddressLine2(getDefaultDeliveryAddress()!)}</div>{getDefaultDeliveryAddress()?.country ? (
  <div className="text-gray-600">{getDefaultDeliveryAddress()?.country}</div>
) : null}</div>
) : null}{!getDefaultDeliveryAddress() ? (
