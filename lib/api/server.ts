 ...
 * and Server Actions.
 *
 * Counterpart to the client-side `graphqlClient` + `getServices` in
 * `lib/api.ts`. The client client points at the relative `/api/graphql`
 * proxy (which works in the browser, not on the server) and resolves the
 * Bearer token via `localStorage` (which doesn't exist on the server).
 *
 * This module instead:
 *  - Talks **directly** to the upstream Propeller GraphQL endpoint
 *    (`BOILERPLATE_GRAPHQL_ENDPOINT`) using `securityMode: 'direct'` and the
 *    server-only `BOILERPLATE_API_KEY`. We don't go through our own proxy
 *    from the server — the proxy is a public-surface shield for clients,
 *    not a same-server hop we should pay for on every RSC render.
 ...
 *
 *   import { getServerInfra, fetchProduct } from '@/lib/api/server';
...
import { getServices, toPlain, type Services } from '../api';
...
if (typeof window === 'undefined' && !GRAPHQL_ENDPOINT) {
  ...
  console.warn(
    '[lib/api/server] BOILERPLATE_GRAPHQL_ENDPOINT is empty. Server-side ' +
...
// ── Server-side services accessor (mirrors client-side getServices) ──────────
...
export function getServerServices(client?: GraphQLClient): Services {
  return getServices(client ?? createServerClient());
}
