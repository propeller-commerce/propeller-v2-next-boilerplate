 ...
 * through here keeps one order in one place.
 */
...
const getChannelDefaults = unstable_cache(
  async (channelId: number): Promise<ChannelDefaults> => {
    try {
      ...
      // sent integrators hunting through channel config.
      //
 ...
 * the API and surface as an unexplained "Failed to load menu".
 *
 ...
 * them — so a visitor saw products they should not.
 *
...
async function listingUserId(infra: ServerInfra): Promise<number | undefined> {
  ...
  // Deliberately NOT guarded: a failed channel lookup throws
