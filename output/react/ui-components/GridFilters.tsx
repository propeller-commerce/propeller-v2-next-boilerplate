...
return (filter as AttributeFilter)?.attributeDescription?.descriptions?.[0]?.value || (filter as AttributeFilter)?.attributeDescription?.name || '';
}
...
return ((filter as AttributeFilter)?.textFilters as any[] || []).filter((o: any) => (o?.count || 0) > 0 || (o?.countActive || 0) > 0);
}
...
function handleCheckbox(filter: AttributeFilter, value: string, checked: boolean): ReturnType<GridFiltersState["handleCheckbox"]>{
...
if (changed) setExpandedFilters(nextExp)
    ...
    [props.filters]);useEffect(() => {
      setCurrentMin(props.priceMin as number || 0);
setCurrentMax(props.priceMax as number || 9999)
    },
    [props.priceMin, props.priceMax]);useEffect(() => {
...
return (
  ...
  <div  className={`space-y-4 ${props.isMobile as boolean ? 'pb-8' : 'sticky top-24'} ${props.className as string || ''}`}>{showPriceFilter() && (props.priceMin !== undefined || props.priceMax !== undefined) ? (
  <><div className="space-y-3"><h3 className="text-xs font-semibold uppercase tracking-wide text-gray-500">
                    Price Range
                </h3><div className="flex items-center gap-2"><div className="relative flex-1"><span className="absolute left-2.5 top-1/2 -translate-y-1/2 text-xs text-gray-400 pointer-events-none">€</span><input  type="number" className="w-full pl-6 pr-2 h-8 rounded-md border border-gray-200 bg-white text-sm focus:outline-none focus:ring-1 focus:ring-violet-500"  value={currentMin}  min={getMinBound()}  max={getMaxBound()}  onChange={(e) => handleMinChange(parseFloat(e.target.value) || 0) }  onBlur={(event) => applyPrice() }  /></div><span className="text-gray-400 text-sm select-none">–</span><div className="relative flex-1"><span className="absolute left-2.5 top-1/2 -translate-y-1/2 text-xs text-gray-400 pointer-events-none">€</span><input  type="number" className="w-full pl-6 pr-2 h-8 rounded-md border border-gray-200 bg-white text-sm focus:outline-none focus:ring-1 focus:ring-violet-500"  value={currentMax}  min={getMinBound()}  max={getMaxBound()}  onChange={(e) => handleMaxChange(parseFloat(e.target.value) || 0) }  onBlur={(event) => applyPrice() }  /></div></div><div className="relative h-4 pt-1"><input  type="range" className="absolute w-full h-1.5 bg-transparent appearance-none pointer-events-none [&::-webkit-slider-thumb]:pointer-events-auto [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-4 [&::-webkit-slider-thumb]:h-4 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-violet-600 [&::-webkit-slider-thumb]:cursor-pointer z-20"  min={getMinBound()}  max={getMaxBound()}  value={currentMin}  onChange={(e) => {
handleMinChange(parseFloat(e.target.value));
applyPrice();
} }  /><input  type="range" className="absolute w-full h-1.5 bg-transparent appearance-none pointer-events-none [&::-webkit-slider-thumb]:pointer-events-auto [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-4 [&::-webkit-slider-thumb]:h-4 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-violet-600 [&::-webkit-slider-thumb]:cursor-pointer z-20"  min={getMinBound()}  max={getMaxBound()}  value={currentMax}  onChange={(e) => {
handleMaxChange(parseFloat(e.target.value));
applyPrice();
} }  /><div className="absolute top-1.5 left-0 right-0 h-1.5 bg-gray-200 rounded z-10"  /></div></div>
<div className="h-px bg-gray-100"  /></>
) : null}{(props.filters as AttributeFilter[]).length === 0 ? (
  <p className="text-sm text-gray-400 italic">No filters available</p>
) : null}{getFilteredFilters()?.map((filter) => (
  <div className="border-b border-gray-100 pb-3 last:border-b-0"  key={getFilterName(filter)}><button  type="button" className="w-full flex items-center justify-between gap-2 text-left py-1 hover:text-violet-600 transition-colors"  onClick={(event) => toggleAccordion(getFilterName(filter)) }><span className="text-sm font-semibold text-gray-700 truncate">{getFilterTitle(filter)}</span><svg  fill="none"  stroke="currentColor"  viewBox="0 0 24 24"  className={`h-4 w-4 flex-shrink-0 text-gray-400 transition-transform duration-200 ${isExpanded(getFilterName(filter)) ? 'rotate-180' : ''}`}><path  strokeLinecap="round"  strokeLinejoin="round"  d="M19 9l-7 7-7-7"  strokeWidth={2}  /></svg></button>{isExpanded(getFilterName(filter)) ? (
  <div className="pt-2 space-y-1.5">{getValidOptions(filter)?.map((option) => (
  <label className="flex items-center gap-2 cursor-pointer group"  key={option.value}><input  type="checkbox" className="h-4 w-4 rounded border-gray-300 text-violet-600 focus:ring-violet-500 cursor-pointer flex-shrink-0"  checked={isSelected(getFilterName(filter), option.value)}  onChange={(e) => handleCheckbox(filter, option.value, e.target.checked) }  /><span className="flex-1 text-sm text-gray-600 leading-none select-none group-hover:text-gray-900">{option.value}<span className="ml-1 text-xs text-gray-400">
                                                ({getCount(option)})
                                            </span></span></label>
))}</div>
) : null}</div>
