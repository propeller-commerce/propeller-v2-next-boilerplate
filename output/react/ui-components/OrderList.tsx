...
function OrderList(props: OrderListProps) {
  ...
  const [totalItems, setTotalItems] = useState<OrderListState["totalItems"]>(
    () => 0
  );

  const [currentPage, setCurrentPage] = useState<OrderListState["currentPage"]>(
    () => 1
  );

  const [itemsPerPage, setItemsPerPage] = useState<
    OrderListState["itemsPerPage"]
  >(() => props.initialItemsPerPage || 10);

  const [totalPages, setTotalPages] = useState<OrderListState["totalPages"]>(
    () => 0
  );

  const [rowsClickable, setRowsClickable] = useState<
    OrderListState["rowsClickable"]
  >(() => props.rowsClickable || false);

  const [fetching, setFetching] = useState<OrderListState["fetching"]>(
    () => false
  );
  ...
  async function fetchOrders(
    ...
    try {
      ...
      setTotalPages(
        Math.ceil((response.itemsFound || 0) / (response.offset || 10))
      );
    } catch (error) {
  ...
  function formatPrice(
    price: number
  ): ReturnType<OrderListState["formatPrice"]> {
                      ...
                      <input
                        ...
                        value={
                          searchForm.createdAt?.greaterThan
                            ? (
                                searchForm.createdAt.greaterThan as string
                              ).split("T")[0]
                            : ""
                        }
                        onChange={(e) => {
                      ...
                      <input
                        ...
                        value={
                          searchForm.createdAt?.lessThan
                            ? (searchForm.createdAt.lessThan as string).split(
                                "T"
                              )[0]
                            : ""
                        }
                      ...
                      <input
                        ...
                        value={
                          searchForm.lastModifiedAt?.greaterThan
                            ? (
                                searchForm.lastModifiedAt.greaterThan as string
                              ).split("T")[0]
                            : ""
                        }
                        onChange={(e) => {
                      ...
                      <input
                        ...
                        value={
                          searchForm.lastModifiedAt?.lessThan
                            ? (
                                searchForm.lastModifiedAt.lessThan as string
                              ).split("T")[0]
                            : ""
                        }
                        onChange={(e) => {
                  ...
                  {field === "sortInput" ? (
                    <div className="flex space-x-2 w-full">
                      ...
                      >
                        ...
                        {Object.values(Enums.OrderSortField)?.map(
                          (sortField) => (
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {orders?.map((order) => (
                      <tr
                        ...
                        key={order.id}
                        onClick={(event) =>
                          rowsClickable && props.onOrderClick(order.id)
                        }
                      >
                        {columns?.map((col) => (
                          <td
