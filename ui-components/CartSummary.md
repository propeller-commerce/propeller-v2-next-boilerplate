...
The CartSummary component displays a structured overview of shopping cart totals: subtotal, discounts, shipping costs, VAT breakdowns, and the final total. It receives a `Cart` object as a prop and renders each line item with granular visibility controls. An optional checkout button can trigger navigation or custom checkout logic.
