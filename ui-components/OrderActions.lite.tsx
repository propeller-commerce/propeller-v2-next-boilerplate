...
export default function OrderActions(props: OrderActionsProps) {
    const state = useStore<OrderActionsState>({
        reordering: false,
        downloading: false,
        toastMessage: '',
        toastType: '',
        ...
        async handleDownloadPDF() {
            ...
            try {
                ...
                if (!pdfResponse) {
                    state.showToast(state.getLabel('pdfError', 'Failed to download PDF'), 'error');
                    return;
                }
                ...
                state.showToast(state.getLabel('pdfSuccess', 'PDF downloaded successfully'), 'success');
            } catch (error) {
        ...
        async handleReorder() {
            ...
            try {
                ...
                if (!cartId) {
                    ...
                    if (!cartId) {
                        state.showToast(state.getLabel('noCartId', 'Could not create or find a cart'), 'error');
                        return;
                    }
                }
                ...
                for (let i = 0; i < parentItems.length; i++) {
                    ...
                    const isCluster =
                        ...
                        typeof item.product.cluster === 'object';
                ...
                state.showToast(state.getLabel('reorderSuccess', 'All items added to cart'), 'success');
            } catch (error) {
                console.error('Error during re-order:', error);
                state.showToast(state.getLabel('reorderError', 'Failed to add items to cart'), 'error');
            } finally {
            ...
            <div className="flex flex-row items-center gap-3 flex-shrink-0">
                ...
                >
                    <Show when={state.downloading}>
                        {state.getLabel('downloadingPdf', 'Downloading...')}
                    </Show>
                    <Show when={!state.downloading}>
                        {state.getLabel('downloadPdf', 'Order confirmation (PDF)')}
                    </Show>
                </button>
                <button
                    type="button"
                    onClick={() => state.handleReorder()}
                    disabled={state.reordering}
                    className="text-primary hover:text-primary/80 text-sm font-medium hover:underline disabled:opacity-50 disabled:cursor-not-allowed"
                >
                    <Show when={state.reordering}>
                        {state.getLabel('reordering', 'Adding items...')}
                    </Show>
                    <Show when={!state.reordering}>
                        {state.getLabel('reorder', 'Order again')}
                    </Show>
                </button>
            </div>
